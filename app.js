...
class Queen {
    ...
    isValid = async (board, r, col, n) => {
        ...
        for (let i = r - 1; i >= 0; --i) {
            ...
            column.style.backgroundColor = "#7eff3e";
            await q.delay();
        }
    ...
    clearColor = async (board) => {
        for (let j = 0; j < n; ++j) {
            ...
            for (let k = 0; k < n; ++k)
                (j + k) & 1
                    ? (row.getElementsByTagName("td")[k].style.backgroundColor = "#9c9894")
                    // ? (row.getElementsByTagName("td")[k].style.backgroundColor = "#ffffff")
                    : (row.getElementsByTagName("td")[k].style.backgroundColor = "#fff");
        }
    }
    ...
    for (let k = 0; k < array[n]; ++k) {
        ...
        for (let i = 0; i < n; ++i) {
            ...
            for (let j = 0; j < n; ++j) {
                ...
                (i + j) & 1
                    ? (col.style.backgroundColor = "#9c9894")//changed to grey
                    : (col.style.backgroundColor = "#a09f9e");
                col.innerHTML = "-";
